...
- Weekly Excel business report and backup export
- Manager email delivery for the weekly Excel report
- Scheduled Sunday weekly email via Vercel Cron
...
RESEND_FROM_EMAIL=Roba Dabo <your-verified-sender@example.com>
...
## 10. Weekly Excel backup/report

CEO/Admin can open **Weekly Backup** in the navigation. The page can generate and download the current week's Excel report to the manager computer and can send the same report by email.

The workbook contains: Weekly Summary, Sales, Employees, Inventory, Expenses, Payments, and Growth.

To enable email delivery, configure `RESEND_API_KEY` and `RESEND_FROM_EMAIL` in Vercel/server environments. The manager recipient is read from `Business Settings → Business Email`. Resend supports spreadsheet attachments in email requests.

The included Vercel Cron runs every Sunday at 20:00 UTC (23:00 Ethiopia time) and calls `/api/cron/weekly-report`. Add `CRON_SECRET` in Vercel so the scheduled endpoint is protected. Vercel cron schedules use UTC and run only from production deployments.

The Excel export is a business-data backup/report, not a complete PostgreSQL database snapshot.
